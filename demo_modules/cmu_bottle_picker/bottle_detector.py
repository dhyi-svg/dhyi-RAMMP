...
class BottleDetector(Node):
    def __init__(self):
        ...
        self.create_subscription(
            Image, self.get_parameter('rgb_topic').value, self._cb_rgb, img_qos, callback_group=self._cb_group
        )
        self.create_subscription(
            Image, self.get_parameter('depth_topic').value, self._cb_depth, img_qos, callback_group=self._cb_group
        )
        self.create_subscription(
    ...
    def _cb_depth(self, msg):
        ...
        self.get_logger().info(f'Depth updated: mean={self.latest_depth_m.mean():.3f}')
    ...
    def _get_centroid_from_depth(self, mask, bbox, depth_m=None):
        ...
        # Build grid of sampled depth pixel coordinates
        vs, us = np.mgrid[d_y1:d_y2:stride, d_x1:d_x2:stride]
        vs = vs.ravel().astype(np.int32)
        us = us.ravel().astype(np.int32)

        zs = depth_m[vs, us].astype(np.float32)
        valid = (zs > min_z) & (zs < max_z)
        vs, us, zs = vs[valid], us[valid], zs[valid]

        if len(zs) == 0:
            self.get_logger().info('points found: 0')
            self.get_logger().info('Not enough masked points: 0')
            return None

        # Unproject depth pixels to depth camera frame, then transform to color camera frame
        Xd = (us - cx_d) * zs / fx_d
        Yd = (vs - cy_d) * zs / fy_d
        pts_d = np.stack([Xd, Yd, zs], axis=1)          # (N, 3)
        ...
        # Project into color image and check mask
        up = np.round(fx_c * (pts_c[:, 0] / Zc) + cx_c).astype(np.int32)
        vp = np.round(fy_c * (pts_c[:, 1] / Zc) + cy_c).astype(np.int32)
        in_bounds = (up >= 0) & (up < Wc) & (vp >= 0) & (vp < Hc)
        pts_c = pts_c[in_bounds]
        up = up[in_bounds]
        vp = vp[in_bounds]

        in_mask = mask[vp, up] > 0
        points_color = pts_c[in_mask]
        ...
        return np.median(points_color, axis=0)
    ...
    def _process_once(self):
        ...
        self.get_logger().info(f'Frame hash: {hash(rgb_snap.tobytes()) % 10000}')
        detections = self._run_yolo(rgb_snap)
        if not detections or detections is None or (isinstance(detections, list) and (len(detections) == 0 or detections[0] is None or detections[0][0] is None)):
        ...
        for mask, bbox, conf, bottom_cut_fraction in detections:
            centroid_cam = self._get_centroid_from_depth(mask, bbox, depth_m=depth_snap)
